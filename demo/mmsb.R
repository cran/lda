...
set.seed(8675309)
  ...
  memberships <- cbind(data.frame(name=colnames(sampson$SAMPLK3)),
                       memberships)
  colors <- with(memberships, rgb(theta.3, theta.1, theta.2))
  ...
  plot.1 <- qplot(x = c(0, 0, 1),  y = c(0, 1, 0),
              ...
              geom="segment") + 
        geom_point(aes(x=theta.1, y=theta.3), colour = I(colors),
                   data = memberships)  +
        scale_x_continuous(breaks=seq(0, 1, length.out=5),
                           limits = c(-0.25, 1.25)) +
        scale_y_continuous(breaks=seq(0, 1, length.out=5),
                           limits = c(-0.25, 1.25)) +
        geom_text(aes(x=theta.1, y=theta.3, label=name), 
                  colour = I(colors),
                  data = memberships,
                  angle=angles * 180 / pi, size=2, hjust=-0.5) +
